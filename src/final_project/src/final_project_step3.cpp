...
private:
        ...
        const rclcpp_action::GoalUUID &uuid, std::shared_ptr<const MoveTurtle::Goal> goal)
    {
        ...
        if(!server_activated_){
            RCLCPP_INFO(this->get_logger(), "action server not activated jet");
        ...
        //Goal policy:
        {
            ...
            if(goal_handle_){
                if(goal_handle_->is_active()){
                    RCLCPP_INFO(this->get_logger(), "current goal still active");
                    return rclcpp_action::GoalResponse::REJECT;
                }
            }
        }
        ...
        if(fabs(goal->linear_vel_x) > 3.0 ||
           ...
           (goal->duration_sec <= 0.0)){
            RCLCPP_INFO(this->get_logger(), "Invalid goal");
            return rclcpp_action::GoalResponse::REJECT;
        }
        ...
        const std::shared_ptr<MoveTurtleGoalHandle> goal_handle)
    {
        ...
        while (rclcpp::ok())
        {
            ...
            if(!server_activated_){
                ...
                result->message = "Aborted because server was deactived";
